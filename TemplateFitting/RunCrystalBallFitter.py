...
os.system('mkdir Logs_CB FitterOutputs_CB Plots_CB')
os.system('mkdir Logs_CB/'+CRdef+' FitterOutputs_CB/'+CRdef+' Plots_CB/'+CRdef)
os.system('rm Logs_CB/'+CRdef+'/* Plots_CB/'+CRdef+'/* FitterOutputs_CB/'+CRdef+'/*')
...
for Datasets in YearCombinations:
  ...
  for case in nJetCases:
    ...
    command += 'python CrystalBallFitter.py --years '+DataPeriod+' --njetFlag '+case+' --muFlag '+mu+' --CRdef '+CRdef+' --doSystematics '+doSystematics+' > Logs_CB/'+CRdef+'/Data'+DataPeriod+'_njet'+case+'_mu'+mu+extra+' 2>&1 && '
command = command[:-2]
print(command)
os.system(command)
